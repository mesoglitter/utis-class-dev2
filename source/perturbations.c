...
int perturbations_total_stress_energy(
  ...
  /* ========================================================= */
  /* UTIS: local clustering fraction for stress-energy source   */
  /* ========================================================= */
  ...
  if (_scalars_) {
    ...
    /* cdm contribution */
    if (pba->has_cdm == _TRUE_) {
      ppw->delta_rho += utis_current_clustering*ppw->pvecback[pba->index_bg_rho_cdm]*y[ppw->pv->index_pt_delta_cdm]; // UTIS-scaled CDM contribution to total perturbed stress-energy
      if (ppt->gauge == newtonian)
        ppw->rho_plus_p_theta = ppw->rho_plus_p_theta + utis_current_clustering*ppw->pvecback[pba->index_bg_rho_cdm]*y[ppw->pv->index_pt_theta_cdm]; // UTIS-scaled contribution to total perturbed stress-energy

      ppw->rho_plus_p_tot += ppw->pvecback[pba->index_bg_rho_cdm];

      if (ppt->has_source_delta_m == _TRUE_) {
        delta_rho_m += utis_current_clustering*ppw->pvecback[pba->index_bg_rho_cdm]*y[ppw->pv->index_pt_delta_cdm]; // UTIS-scaled CDM contribution to delta rho_matter
        rho_m += ppw->pvecback[pba->index_bg_rho_cdm];
      }
      if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
        if (ppt->gauge == newtonian)
          rho_plus_p_theta_m += utis_current_clustering*ppw->pvecback[pba->index_bg_rho_cdm]*y[ppw->pv->index_pt_theta_cdm]; // UTIS-scaled contribution to [(rho+p)theta]_matter
...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_cdm == _TRUE_) {
      ...
      /* ========================================================= */
      /* UTIS: local clustering fraction fetch                     */
      /* ========================================================= */
      double utis_current_clustering = 1.0;
      if (pba->utis.has_utis == _TRUE_) {
        utis_current_clustering = ppw->pvecback[pba->index_bg_utis_clustering_fraction];
      }
